...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  @media (min-width: 640px) { .caption { gap: 1.5rem; padding: 0 2rem 1.75rem; } }
  .caption .inner { flex: 1; min-width: 0; }

  /* Step controls sit at the outer edges, the title between them. */
  .step {
    flex-shrink: 0;
    display: flex;
    align-items: center;
  ...
  .step:hover:not(:disabled) { border-color: color-mix(in oklab, var(--ink) 40%, transparent); background: color-mix(in oklab, var(--ink) 5%, transparent); }
  .step:disabled { cursor: default; border-color: color-mix(in oklab, var(--ink) 5%, transparent); color: color-mix(in oklab, var(--ink-faint) 40%, transparent); }
  ...
  /*
    Leaving animates on this element alone. Fading this *and* the sheet inside
    it meant two nested opacity layers compositing at once, over a subtree
    holding a photograph and the map — which is what made the exit drag.
    One layer moves; everything within it is static.
  */
  .record-scroll {
    position: fixed;
    inset: 0;
    ...
    opacity: 0;
    translate: 0 44px;
    transition: opacity 340ms var(--ease), translate 420ms var(--ease);
  ...
  .record[data-open] .record-scroll { opacity: 1; translate: 0 0; }
  ...
  /* Static: the scroll container above carries the whole record in and out. */
  .sheet {
    border: 1px solid var(--line);
    border-radius: 1rem;
    background: var(--card);
...
<div class="stage">
  ...
  <div class="skyline">
    ...
    <div class="caption">
      <button class="step" id="stepPrev" type="button">←</button>
      <div class="inner" id="caption"></div>
      <button class="step" id="stepNext" type="button">→</button>
    </div>
  </div>
</div>
...
(() => {
  ...
  let activeSlug = null;
  let userMoved = false;
  /** Destination of a deliberate step, held until the skyline reaches it. */
  ...
  function paint() {
    ...
    // Hold a deliberate step's destination until the skyline reaches it, then
    // fall through so the arrival is synced like any other move.
    if (travellingTo) {
      if (best !== travellingTo) return;
      travellingTo = null;
    }
  ...
  viewport.addEventListener("wheel", (event) => {
    ...
    userMoved = true;
    travellingTo = null; // taking the wheel abandons any step in progress
  ...
  /**
   * Steps through the surviving matches, so an arrow never lands on a ghost.
   * Named stepBy, not step: the spring's own step(now) is declared above, and
   * function hoisting would let a second step silently replace it.
   */
  function stepBy(direction) {
    ...
    travellingTo = next.slug;
    activeSlug = next.slug;
    syncActive();
    renderCaption();
    target = clamp(offset);
    if (reduced || document.hidden) jumpTo(offset); else run();
  }

  /** Reflects the current position on the two edge controls. */
  function syncActive() {
    const order = MONUMENTS.filter((m) => matched.has(m.slug));
    const index = order.findIndex((m) => m.slug === activeSlug);
    const earlier = index > 0 ? order[index - 1] : null;
    const later = index >= 0 && index < order.length - 1 ? order[index + 1] : null;

    for (const [slug, li] of nodes) {
      if (slug === activeSlug) li.setAttribute("data-active", "");
      else li.removeAttribute("data-active");
    }

    stepPrev.disabled = !earlier;
    stepNext.disabled = !later;
    stepPrev.setAttribute("aria-label", earlier ? "Earlier: " + earlier.name + ", " + earlier.yearLabel : "No earlier monument");
    stepNext.setAttribute("aria-label", later ? "Later: " + later.name + ", " + later.yearLabel : "No later monument");
    if (earlier) stepPrev.title = earlier.name; else stepPrev.removeAttribute("title");
    if (later) stepNext.title = later.name; else stepNext.removeAttribute("title");
  }

  stepPrev.addEventListener("click", () => stepBy(-1));
